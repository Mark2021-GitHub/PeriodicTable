...
function setupElement() {
  ...
  n++;
  elements[11] = new Element(11,"Na","Sodium (Natrium)","소듐 / 나트륨",1,3, m_al, [2,8,1,0,0,0,0], solid, "1족 알칼리 금속인 소듐은 물과 만나면 리튬처럼 발열하며 수소를 발생시키고 폭발을 일으킨다. 반응성과 폭발력은 리튬보다 강해서 10cm정도의 소듐 막대를 물에 던지면 3m에 이르는 폭발이 일어난다. 또한 산소와 만나면 반응성이 낮은 산화소듐으로 변하므로 소듐은 물과 공기에 닿지 않도록 석유에 담가 보관한다. 소듐은 포타슘과 함께 뇌로 신경 자극을 전달하며, 세포의 삼투압을 조절한다. 바닷물 속 소금에 풍부하게 존재하는 무른 금속");
  n++;
  elements[12] = new Element(12,"Mg","Magnesium","마그네슘",2,3, m_ae, [2,8,2,0,0,0,0],  solid, "연소할 때 밝은 백색광을 내므로 섬광탄, 폭죽, 초창기 카메라 플래쉬 등으로 활용되는 금속, 탄산마그네슘 화합물은 습기 제거 기능이 있어서 암벽 등반가나 운동선수들이 손에 바르는 분말로 사용된다.");
  n++;
  elements[13] = new Element(13,"Al","Aluminium","알루미늄",13,3, m_pt, [2,8,3,0,0,0,0],  solid, ": 가볍고, 열 전도성이 좋아 다양한 합금 소재로 산업에서 널리 활용되는 금속");
  n++;
  elements[14] = new Element(14,"Si","Silicon","규소 / 실리콘",14,3, ml, [2,8,4,0,0,0,0], solid,  ": 지각에 산소 다음으로 풍부하게 존재하는 가장 저렴한 반도체");
  n++;
  elements[15] = new Element(15, "P", "Phosphorus", "인", 15, 3, nm_re, [2,8,5,0,0,0,0], solid,  ": 소변에서 추출되는 생체 화합물의 필수 원소인 비금속");
  n++;
  elements[16] = new Element(16, "S", "Sulfur", "황", 16, 3, nm_re, [2,8,6,0,0,0,0],  solid, "화산지대에서 직접 채굴 되는 냄새가 지독한 노란색 비금속인 황은 살충제, 살균제, 파마 약, 의약품, 축전지 등 산업 분야에 널리 쓰인다. 황화합물인 황화수소는 가수 누출을 냄새를 감지하는데 이용된다. 지옥을 유황으로 된 끓는 연못이라고 묘사한 성경 구절을 근거로 지옥의 온도는 황의 긇는점 444.6도로 추론한다.  "); 
  n++;
  elements[17] = new Element(17, "Cl", "Chlorine", "염소", 17, 3, nm_ha, [2,8,7,0,0,0,0], gas,  "1차 세계대전 때 독가스로 사용된 무시무시한 독성을 가진 기체. 오늘날에는 살균, 소독을 위해 수돗물과 수영장 물에 쓰인다. '락스'라고 알려진 염소가 포함된 표백제는 옷을 몇 분만에 표백시킨다. 전세계적으로 사용되어 온 DDT 라는 염소 기반 농약은 가장 효과적인 살충제로 많이 이용됐는데 심각한 환경오염과 생태계를 파괴하는 사실이 뒤늦게 밝혀져 지금은 거의 사용하지 않는다. 상온에서 자극적인 냄새가 있는 황록색 기체로 존재한다.");
  n++;
  elements[18] = new Element(18, "Ar", "Argon", "아르곤", 18, 3, nm_ng, [2,8,8,0,0,0,0], gas, ": 반응성이 거의 없는 게으른 기체");
  n++;
  elements[19] = new Element(19, "K","Potassium (Kalium)","포타슘 / 칼륨",1,4, m_al, [2,8,8,1,0,0,0], solid, "무른 알칼리 금속인 포타슘은 리튬과 소듐처럼 물과 반응해 폭발하는데, 반응성은 두 원소보다 훨씬 높다. 포타슘은 손 위에 올려놓기만 해도 손의 수분과 반응해 불이 붙고 폭발하므로 특히 주의해서 다뤄야 한다.질소, 인과 함께 비료의 3요소로 꼽히는 포타슘을 너무 많이 섭취하면 심장마비가 올 수 있다. 염화포타슘은 심장 수술을 할 때 일시적으로 심장을 멈추는 심정지액으로  쓰인다.");
  n++;
  ...
  n++;
  elements[22] = new Element(22, "Ti", "Titanium", "타이타늄", 4, 4, m_tr, [2,8,10,2,0,0,0], solid,  ": 금속(전이 금속)");
  n++;
  elements[23] = new Element(23, "V", "Vanadium", "바나듐", 5, 4, m_tr, [2,8,11,2,0,0,0], solid,  ": 금속(전이 금속)");
  n++;
  elements[24] = new Element(24, "Cr", "Chromium", "크로뮴", 6, 4, m_tr, [2,8,13,1,0,0,0], solid,  ": 금속(전이 금속)");
  n++;
  elements[25] = new Element(25,"Mn","Manganese","망가니즈 / 망간", 7, 4, m_tr, [2,8,13,2,0,0,0],  solid, ": 금속(전이 금속)");
  n++;
  elements[26] = new Element(26, "Fe", "Iron (Ferrum)", "철", 8, 4, m_tr, [2,8,14,2,0,0,0], solid, ": ");
  n++;
 elements[27] = new Element(27, "Co", "Cobalt", "코발트", 9, 4, m_tr, [2,8,15,2,0,0,0],  solid, ": ");
  n++;
  elements[28] = new Element(28, "Ni", "Nickel", "니켈", 10, 4, m_tr, [2,8,16,2,0,0,0],  solid, ": ");
  n++;
  elements[29] = new Element(29,"Cu","Copper (Cuprum)","구리",11,4, m_tr, [2,8,18,1,0,0,0],  solid, "항균성이 뛰어난 금속으로 손잡이나 버튼등 사람 손이 많이 타는 곳에 구리가 쓰인다. 동전에도 알루미늄, 아연 등과 함께 많은 양의 구리가 들어 있다. 전기 전도성과 열 투과성이 뛰어나고, 은,알루미늄,금 보다는 저렴하기 때문에 전선과 전자 회로에 많이 쓰인다.");
  n++;
  elements[30] = new Element(30, "Zn", "Zinc", "아연", 12, 4, m_tr, [2,8,18,2,0,0,0], solid,  "인슐린, 성장 호르몬, 성 호르몬 등의 다양한 효소작용를 돕기 때문에 우리 몸에 반드시 필요한 원소로 생굴과 육류에 풍부하게 존재한다. 아연이 부족하면 면역 기능 저하, 성장이 지연될 수 있다.아연은 기원전부터 사용된 금속 원소로 대표적으로 '함석'이라 불리는 아연 도금된 철강은 내부식 효과가 뛰어나다.또한 최초의 전지인 볼타 전지부터 수은,연료,알칼리 전지까지 수많은 건전지의 음극을 만드는데 아연이 쓰인다.    ");
  n++;
